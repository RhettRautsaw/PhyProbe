...
def Parser(ref):
	...
	tmp = alignfile >> mask(X.ref == ref)
	reads_list = tmp[['qseqid']].drop_duplicates()
	ref_list = tmp >> arrange(X.score, X.length, ascending=False) >> head(1) >> select(X.sseqid)
	#reads_list = alignfile >> mask(X.ref == ref) >> select(X.qseqid)
	#ref_list = best_refs >> mask(X.ref == ref) >> select(X.sseqid)
	
	ref_list.to_csv(file+"_ref.list", header=False, index=False)
...
#best_refs = alignfile >> group_by(X.ref) >> arrange(X.score, X.length, ascending=False) >> head(1)
#alignfile = alignfile[['ref','qseqid']].drop_duplicates()
