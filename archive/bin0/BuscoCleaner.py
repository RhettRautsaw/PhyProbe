...
def mcChecker(locus):
	...
	def RemoveRedundancy(locus):
		first_record = next(SeqIO.parse(locus, "fasta"))
		final = [[first_record.id, first_record.seq]]
		for record in SeqIO.parse(locus,"fasta"):
			ID = str(record.id)
			SEQ = str(record.seq)
			if SEQ == final[0][1] or SEQ in final[0][1] or final[0][1] in SEQ:
				if len(SEQ) >= len(final[0][1]):
					final[0] = [ID, SEQ]
					continue
			if SEQ != final[0][1] and SEQ not in final[0][1] and final[0][1] not in SEQ:
				final.append([ID, SEQ])
				continue
		return final

	sequences = RemoveRedundancy(locus)
	if len(sequences)==1:
		a = open(locus.replace("multi_copy_busco_sequences","single_copy_busco_sequences"), "w")
		a.write(">"+str(sequences[0][0])+"\n"+str(sequences[0][1])+"\n")
		a.close()
...
if len(tmp_count)==0:
	...
	metaeuk_output = glob.glob(folder + "/run*/metaeuk_output/initial_results/*.codon.fas")[0]
