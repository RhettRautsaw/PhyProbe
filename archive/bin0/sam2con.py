...
def main():
	...
	parser.add_argument("-p2", "--prefix2", action="store", dest="prefix2", default="",
		help="Prefix for fasta header, default=input filename without .sam extension")
	...
	parser.add_argument("-f", "--fill", action="store", dest="fill", default="-",
		help="Character for padding regions not covered in the reference, default= -")
	...
	if args.prefix2 == "":
		prefix2 = "".join(args.filename.split("/")[-1]).split(".")[0]
	else:
	...
	for refname in sequences:
		...
		for t in thresholds:
			...
			fasta_header = (">"+prefix2)
			#fasta_header = (">"+prefix2+"|consensus"+str(int(t*100))+" reference:"+refname+
	...
	for reference in fastas:
		outnameprefix  = prefix + "_consensus"
